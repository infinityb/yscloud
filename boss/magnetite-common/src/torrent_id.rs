...
impl FromStr for TorrentId {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        let data = dehex(s, &mut buf[..]).map_err(|()| TorrentIdError)?;

        let tid = TorrentId::from_slice(data).map_err(|_e| TorrentIdError)?;
...
fn dehex<'a>(val: &str, into: &'a mut [u8]) -> Result<&'a [u8], ()> {
