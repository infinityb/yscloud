...
fn check_is_digits(mut digits: &[u8]) -> std::result::Result<&str, Error> {
    if digits.starts_with(b"-") {
        digits = &digits[1..];
    }
    for d in digits {
        if !is_digit(*d) {
            return Err(de::Error::invalid_value(Unexpected::Bytes(digits), &"a valid bencoded integer"));
        }
    }
    Ok(str::from_utf8(digits).unwrap())
}
...
impl<'de> Tokenizer<'de> {
    ...
    pub fn next(&mut self, is_peek: bool) -> IResult<Node<'de>, Error> {
            ...
            if self.scratch_offset < self.scratch.len() {
                return match parse_next_item(&self.scratch[self.scratch_offset..]) {
                    Ok((length, value)) => {
                        ...
                        Ok((length, value.into_owned()))
                    }
                    Err(IErr::Incomplete(length)) => {
                    ...
                    Err(IErr::Error(err)) => Err(IErr::Error(err)),
                };
            }
            if !buf.is_empty() {
                ...
                return Err(IErr::Incomplete(1));
            }
        }
    }
}

fn parse_next_item<'a>(data: &'a [u8]) -> IResult<Node<'a>, Error> {
    let mut rem_iter = data.iter();
    let mut rem_iter_copy = rem_iter.clone();
    match rem_iter.next().unwrap() {
        b'd' => Ok((1, Node::DictionaryStart)),
        b'l' => Ok((1, Node::ArrayStart)),
        b'e' => Ok((1, Node::ContainerEnd)),
        b'i' => match rem_iter_copy.position(|x| *x == b'e') {
            Some(pos) => {
                let rem_slice = rem_iter.as_slice();
                let value = check_is_digits(&rem_slice[..pos - 1])?;
                Ok((1 + pos, Node::Integer(Cow::Borrowed(value))))
            }
            None => Err(IErr::Incomplete(1)),
        },
        b'0'..=b'9' => match rem_iter.position(|x| *x == b':') {
            Some(pos) => {
                ...
                let str_len = check_is_digits(&length_prefix)?;
                let length: usize = str_len.parse()?;
                if rest.len() < length {
                    return Err(IErr::Incomplete(length - rest.len()));
                }
                Ok((
                    pos + length + 1,
...
impl From<std::num::ParseIntError> for IErr<Error> {
    fn from(e: std::num::ParseIntError) -> IErr<Error> {
        IErr::Error(Error {
            offset: 0,
    ...
    Error: From<E1>,
{
    match res {
        Ok((_length, v)) => Ok(v),
        Err(IErr::Incomplete(..)) => Err(Error::truncated(tz)),
        Err(IErr::Error(e)) => Err(e.into()),
    }
}
...
pub type IResult<T, E> = std::result::Result<(usize, T),  IErr<E>>;
